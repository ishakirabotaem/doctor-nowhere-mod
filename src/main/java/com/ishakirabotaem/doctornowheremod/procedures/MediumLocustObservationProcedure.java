...
public class MediumLocustObservationProcedure {
	public static void execute(LevelAccessor world, Entity entity) {
		...
		if (DoctorNowhereModVariables.MapVariables.get(world).locust == false) {
			...
			if (world instanceof ServerLevel _serverLevel) {
				Entity entityinstance = DoctorNowhereModEntities.THE_LOCUST_OBSERVATION.get().create(_serverLevel, null, null, BlockPos.containing(x, y + 1, z), MobSpawnType.MOB_SUMMONED, false, false);
				if (entityinstance != null) {
					entityinstance.setYRot(world.getRandom().nextFloat() * 360.0F);
					DoctorNowhereMod.queueServerWork(6000, () -> {
						if (entityinstance instanceof TheLocustObservationEntity) {
