...
public class MajorLocustProcedure {
	public static void execute(LevelAccessor world, Entity entity) {
		...
		if (DoctorNowhereModVariables.MapVariables.get(world).locust == false) {
			...
			if (world instanceof ServerLevel _serverLevel) {
				Entity entityinstance = DoctorNowhereModEntities.THE_LOCUST.get().create(_serverLevel, null, null, BlockPos.containing(x, y + 1, z), MobSpawnType.MOB_SUMMONED, false, false);
				if (entityinstance != null) {
					...
					DoctorNowhereMod.queueServerWork(1200, () -> {
						if (entityinstance instanceof TheLocustEntity) {
