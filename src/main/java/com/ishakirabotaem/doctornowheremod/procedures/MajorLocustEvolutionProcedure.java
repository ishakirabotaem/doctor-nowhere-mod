...
public class MajorLocustEvolutionProcedure {
	public static void execute(LevelAccessor world, Entity entity) {
		...
		if (DoctorNowhereModVariables.MapVariables.get(world).locuststage == 2) {
			if (DoctorNowhereModVariables.MapVariables.get(world).locust == false) {
				...
				if (random == 0) {
					x = entity.getX() + Mth.nextDouble(RandomSource.create(), -50, -120);
				} else {
					x = entity.getX() + Mth.nextDouble(RandomSource.create(), 50, 120);
				}
				y = 320;
				random = Mth.nextInt(RandomSource.create(), 0, 1);
				if (random == 0) {
					z = entity.getZ() + Mth.nextDouble(RandomSource.create(), -50, -120);
				} else {
					z = entity.getZ() + Mth.nextDouble(RandomSource.create(), 50, 120);
				}
				...
				if (world instanceof ServerLevel _serverLevel) {
					Entity entityinstance = DoctorNowhereModEntities.THE_LOCUST_EVOLUTION.get().create(_serverLevel, null, null, BlockPos.containing(x, y + 1, z), MobSpawnType.MOB_SUMMONED, false, false);
					if (entityinstance != null) {
						...
						if (entity instanceof LivingEntity _livingEntity11 && _livingEntity11.getAttributes().hasAttribute(ForgeMod.SWIM_SPEED.get()))
							_livingEntity11.getAttribute(ForgeMod.SWIM_SPEED.get()).setBaseValue(5);
						DoctorNowhereMod.queueServerWork(1200, () -> {
							if (entityinstance instanceof TheLocustEvolutionEntity) {
